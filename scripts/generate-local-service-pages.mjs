...
const services = [
  {
    ...
    titleTag: (city) => `Balcony Bird Netting ${city} | Pigeon Defenders`,
  ...
  },
  {
    ...
    titleTag: (city) => `Commercial Bird Control ${city} | Pigeon Defenders`,
  ...
  },
  {
    ...
    titleTag: (city) => `Balcony Cleaning ${city} | Pigeon Defenders`,
...
function relatedLinks(current) {
  ...
  const sameService = allPages.filter((page) => page.service.key === current.service.key && page.city.slug !== current.city.slug);
  const all = allPages.filter((page) => page.slug !== current.slug);
  return { sameCity, sameService, all };
}
...
<body>
  ...
  <main>
    ...
    <section class="band"><div class="wrap"><div class="section-head center"><span class="eyebrow">Internal Links</span><h2>Explore more local bird proofing pages.</h2><p>These links help visitors and search engines find the right city and service combination quickly.</p></div><div class="areas">${related.sameService.map((item) => `<a href="${linkFor(item)}"><span>${escapeHtml(item.service.title)} ${escapeHtml(item.city.name)}</span></a>`).join("")}</div><div class="areas local-page-links">${related.all.map((item) => `<a href="${linkFor(item)}"><span>${escapeHtml(item.service.title)} ${escapeHtml(item.city.name)}</span></a>`).join("")}</div></div></section>
...
function sitemapXml() {
  ...
  const local = allPages.map((page) => `  <url><loc>${site}/${page.slug}</loc><lastmod>${lastmod}</lastmod><changefreq>monthly</changefreq><priority>0.65</priority></url>`);
